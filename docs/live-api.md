...
Jika koneksi dan inisialisasi berhasil, Pepebot Server akan membalas dengan status JSON berisikan `{"setupComplete": true}` atau respon ekuivalen yang menandakan Live session siap direkam.

---

## 4. Pengiriman Audio dan Teks

Secara umum, format interaksi berbasis *frame* JSON, atau jika di sisi provider memungkinkan, via *binary frame*.
