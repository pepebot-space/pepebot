...
- **`examples/live-api/paniki_client.py` and `paniki-client.js`**: terminal clients for the same setup. The Python one is full-duplex microphone (`websockets`, `sounddevice`, `numpy`) with barge-in and a mic-silence check that catches a denied macOS microphone permission — which otherwise looks identical to a quiet room. The Node one is text-in and writes each reply to `reply-N.wav`, so `ws` is its only dependency. Both read `/v1/models` and `/v1/voices` at startup and print tool calls as they happen.
